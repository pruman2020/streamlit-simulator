...
# Якщо всі блоки вимкнені — тимчасово активуємо для графіка
active_blocks = [b for b in st.session_state.blocks.values() if b.enabled]
if not active_blocks:
    st.warning("Усі блоки вимкнені. Щоб побачити графіки, увімкніть хоча б один BB.")
else:
    st.markdown("## 📈 Показники ТП (увімкнені блоки)")
    data = {
        "Блок": [],
        "Температура": [],
        "Вологість": [],
    ...
    df_chart = pd.DataFrame(data).set_index("Блок")
    st.line_chart(df_chart[["Температура"]])
    st.line_chart(df_chart[["Вологість"]])
    st.line_chart(df_chart[["Вітер"]])
